...
<div class="pb-8">
    <div class="flex items-center justify-between mb-8 px-8 pt-8">
        <h2 class="text-gray-600 text-2xl font-bold ">Data Model</h2>
    ...
    <div class="bg-white px-8 rounded-lg">
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                ...
                <li aria-current="page">
                    <div class="flex items-center">
                        ...
                        <span class="ms-1 text-sm font-medium text-gray-500 md:ms-2 dark:text-gray-400">Model</span>
                    </div>
                </li>
            </ol>
        </nav>
        <table id="selection-table">
            <thead>
                <tr>
                    <th class="nomor">
                        <span class="flex text-center items-center   text-blue-400 hover:text-blue-600">
                            no
                    ...
                    <th class="aksi text-center justify-end">
                        <span class="flex items-center text-blue-400 hover:text-blue-600">
                            action
                            <svg class="w-4 h-4 ms-1" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m8 15 4 4 4-4m0-6-4-4-4 4" />
            ...
            <tbody>
                ...
                <?php foreach ($models as $model) : ?>
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-800 cursor-pointer">
                        ...
                        <td class="part font-medium text-gray-900 whitespace-nowrap dark:text-white"><?= $model['nama_model'] ?></td>
                        <td class="flex gap-2 justify-end">
                            <button type="button"
                                data-modal-target=modal-edit
                                data-modal-toggle=modal-edit
                                data-tooltip-target="tooltip-edit-<?= $model['id_model'] ?>"
                                data-tooltip-placement="top"
                                class="bg-blue-500 hover:bg-blue-700 p-1"
                                data-model-id="<?= $model['id_model']; ?>"
                                data-model-nama="<?= htmlspecialchars($model['nama_model']); ?>">
                                <svg class="w-5 h-5 text-white " aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.779 17.779 4.36 19.918 6.5 13.5m4.279 4.279 8.364-8.643a3.027 3.027 0 0 0-2.14-5.165 3.03 3.03 0 0 0-2.14.886L6.5 13.5m4.279 4.279L6.499 13.5m2.14 2.14 6.213-6.504M12.75 7.04 17 11.28" />
                                </svg>
                            </button>
                            <div
                                id="tooltip-edit-<?= $model['id_model'] ?>"
                                role="tooltip"
                            ...
                            <a href="#" 
                                ...
                                data-tooltip-target="tooltip-delete-<?= $model['id_model'] ?>"
                                onclick="confirm(<?= $model['id_model'] ?>)">
                                <svg class="w-5 h-5 text-white dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 7h14m-9 3v8m4-8v8M10 3h4a1 1 0 0 1 1 1v3H9V4a1 1 0 0 1 1-1ZM6 7h12v13a1 1 0 0 1-1 1H7a1 1 0 0 1-1-1V7Z" />
                                </svg>
                            </a>
                            <div id="tooltip-delete-<?= $model['id_model'] ?>" role="tooltip" class="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white bg-gray-900 rounded-lg shadow-xs opacity-0 tooltip dark:bg-gray-700">
                                Delete Model
                                <div class="tooltip-arrow" data-popper-arrow></div>
                            </div>
                        </td>
    ...
    document.addEventListener('DOMContentLoaded',() => {
        document.querySelectorAll('[data-modal-target="modal-edit"]').forEach(button => {
            button.addEventListener('click', function() {
                ...
                const model = this.getAttribute('data-model-nama');
    
                console.log(id);
                const modal = document.getElementById('modal-edit');
                modal.querySelector('input[name="model"]').value = model;
                modal.querySelector('form').action = '?page=model&action=update&id=' + id;
            });
        });
    });
    function confirm($id){
        Swal.fire({
            ...
            if (result.isConfirmed) {
                window.location.href = '?page=model&action=delete&id=' + $id;
            }
        })
    }
</script>